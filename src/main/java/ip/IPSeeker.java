 ...
 * 不管为0x1还是0x2，后三个字节都是实际国家名的文件内绝对偏移,如果是地区记录，0x1和0x2的含义不明，但是如果出现这两个字节，也肯定是跟着3个字节偏移，如果不是 则为0结尾字符串
 * 三. "起始地址/结束地址偏移"记录区
...
public class IPSeeker {
    private static final String IP_FILE = IPSeeker.class.getResource("").toString().substring(5, IPSeeker.class.getResource("").toString().indexOf("javaweb_maven")) + "javaweb_maven/configuration/qqwry.dat";
    // 一些固定常量，比如记录长度等等
    ...
    private IPSeeker() {
        ...
        if (ipFile != null) {
            try {
                ...
                System.out.println("IP地址信息文件数据格式有错误，IP定位功能将无法使用");
                ipFile = null;
            }
        }
    }
    ...
    /**
     * 给定一个地点的不完全名字，得到一系列包含areaName子串的IP范围记录
     *
     * @param areaName 地点子串
     * @return 包含IPEntry类型的List
     */
    public List getIPEntriesDebug(String areaName) {
        ...
        for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
            ...
            if (temp != -1) {
                ...
                if (loc.country.indexOf(areaName) != -1 || loc.area.indexOf(areaName) != -1) {
     ...
     * @return 包含IPEntry类型的List
     */
    public List getIPEntries(String areaName) {
        List ret = new ArrayList();
        try {
            // 映射IP信息文件到内存中
            if (mbb == null) {
            ...
            for (int offset = (int) ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
                ...
                if (temp != -1) {
                    IPLocation loc = getIPLocation(temp);
                    // 判断是否这个地点里面包含了s子串，如果包含了，添加这个记录到List中，如果没有，继续
                    if (loc.country.indexOf(areaName) != -1
                            || loc.area.indexOf(areaName) != -1) {
                        IPEntry entry = new IPEntry();
                        entry.country = loc.country;
                        entry.area = loc.area;
                        // 得到起始IP
